...
namespace FlyEase_ApiRest_.Controllers
{
    ...
    public class VuelosController : CrudController<Vuelo, int, FlyEaseDataBaseContextAuthentication>
    {
        ...
        [SwaggerResponse((int)HttpStatusCode.BadRequest, "No se ha encontrado el vuelo, rectifique los datos", typeof(string))]
        [SwaggerResponse((int)HttpStatusCode.Conflict, "Conflicto con la base de datos, no se encontraron asientos disponibles para este avion.", typeof(string))]
            ...
            .ToListAsync();
                ...
                if (vuelo != null)
                {
                    if (vuelo.Avion.Asientos.Count > 0)
                    {
                        ...
                        return StatusCode(StatusCodes.Status200OK, new { mensaje = "ok", Succes = true, response = new { AsientosOcupados, AsientosDisponibles, AsientosTotales } });
                    }
                    else
                    {
                        return StatusCode(StatusCodes.Status409Conflict, new { mensaje = "El avion no posee asientos", Succes = false });
                    }
                }
                else
                {
                    return StatusCode(StatusCodes.Status400BadRequest, new { mensaje = "Vuelo no encontrado o no disponible", Succes = false });
                }
