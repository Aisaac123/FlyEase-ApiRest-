...
public class AuthenticationTokenService : IAuthentication
{
    ...
    public async Task<AuthenticationResponse> GetRefreshToken(int idusuario, RefreshTokenRequest refreshtoken = null)
    {
        ...
        if (refreshTokenEncontrado == null)
            return new AuthenticationResponse { Succes = false, Msg = "No existe refresh Token" };
    ...
    public async Task<AuthenticationResponse> GetToken([FromBody] Administrador admin = null)
    {
        try
        {
            if (admin == null)
            {
                ...
                {
                    Msg = "Cliente Valido", Succes = true,
                    Tokens = { AdminAuthentication = false, PrimaryToken = token }
                };
            }
            ...
            if (!AdminExist)
            {
                return new AuthenticationResponse
                {
                    Msg = "No encontrado", Succes = false,
                    Tokens = { AdminAuthentication = false, PrimaryToken = null, RefreshToken = null }
                };
            }

            {
                ...
                return new AuthenticationResponse
                {
                    Msg = "Administrador Valido", Succes = true,
                    Tokens = { AdminAuthentication = true, PrimaryToken = token, RefreshToken = refreshtoken }
                };
            }
        }
        catch (Exception ex)
    ...
    private async Task<AuthenticationResponse> SaveRefreshToken(int idAdmin, string token, string refreshToken)
    {
        ...
        return new AuthenticationResponse
        {
            Tokens = new TokenClass { PrimaryToken = token, RefreshToken = refreshToken, AdminAuthentication = true },
            Succes = true, Msg = "Ok"
        };
    }
}