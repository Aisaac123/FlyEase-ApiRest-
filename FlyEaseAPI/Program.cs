...
{
    opt.AddPolicy(CorsRules, builder =>
    {
...
builder.Services.AddDbContext<FlyEaseDataBaseContextAuthentication>(con =>
    con.UseNpgsql(builder.Configuration.GetConnectionString("Fl0ServerConnection")));
...
builder.Services.AddSwaggerGen(c =>
{
    ...
    {
        Description =
            "Autorizacion JWT usando el esquema Bearer, Ingresa un token valido.\n si todavia no tienes uno, puedes solicitarlo con los metodos:" +
...
{
    if (context.Request.Path == "/swagger" || context.Request.Path == "//" || context.Request.Path == "/s" ||
        context.Request.Path == "/S")
    {
