...
namespace FlyEase_ApiRest_.Controllers
{
    namespace TuNamespace
    {
        ...
        public class AerolineasController : CrudController<Aerolinea, int, FlyEaseDataBaseContextAuthentication>
        {
            ...
            public AerolineasController(FlyEaseDataBaseContextAuthentication context,
            ...
            protected override async Task<string> InsertProcedure(Aerolinea entity)
            {
                try
                {
                    ...
                    {
                        new("nombre_aereolinea", entity.Nombre),
                        new("v_codigo_iata", entity.Codigoiata),
                        new("v_codigo_icao", entity.Codigoicao)
                    };

                    await _context.Database.ExecuteSqlRawAsync(
                        "CALL p_insertar_aereolinea(@nombre_aereolinea, @v_codigo_iata, @v_codigo_icao)", parameters);
                    return "Ok";
                }
                catch (Exception ex)
                {
            ...
            protected override async Task<string> DeleteProcedure(int id_Aerolinea)
            {
                try
                {
                    var parameters = new NpgsqlParameter[]
                    {
                        new("id_aereolinea", id_Aerolinea)
                    };

                    await _context.Database.ExecuteSqlRawAsync("CALL p_eliminar_aereolinea(@id_aereolinea)",
                        parameters);
                    return "Ok";
                }
                catch (Exception ex)
            ...
            protected override async Task<string> UpdateProcedure(Aerolinea nuevaAerolinea, int id_Aerolinea)
            {
                try
                {
                    ...
                    {
                        new("id_aereolinea", id_Aerolinea),
                        new("nuevo_nombre", nuevaAerolinea.Nombre),
                        new("nuevo_codigo_iata", nuevaAerolinea.Codigoiata),
                        new("nuevo_codigo_icao", nuevaAerolinea.Codigoicao)
                    };

                    await _context.Database.ExecuteSqlRawAsync(
                        "CALL p_actualizar_aereolinea(@id_aereolinea, @nuevo_nombre, @nuevo_codigo_iata, @nuevo_codigo_icao)",
                        parameters);
                    return "Ok";
                }
                catch (Exception ex)
