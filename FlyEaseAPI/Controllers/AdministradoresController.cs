...
public class AdministradoresController : ReadController<Administrador, int, FlyEaseDataBaseContextAuthentication>
{
    ...
    [SwaggerOperation(
        "Autenticar y autorizar un administrador por medio de JWT (Json Web Token) usando sus credenciales de usuario y contraseña.")]
    [SwaggerResponse(StatusCodes.Status200OK, "Operación exitosa", typeof(AuthenticationResponse))]
    [SwaggerResponse(StatusCodes.Status401Unauthorized, "Administrador no autorizado: credenciales erroneas o inactivo",
        typeof(string))]
    [SwaggerResponse(StatusCodes.Status500InternalServerError, "Error interno del servidor", typeof(string))]
    public async Task<IActionResult> Authentication([FromBody] Administrador admin)
    {
        try
        {
            ...
            if (!Aut.Succes) return StatusCode(StatusCodes.Status401Unauthorized, new { response = Aut });
    ...
    [SwaggerOperation(
        "Refrescar un nuevo token de actualización por medio de RT (Refresh Token). Recibe el token expirado y el refresh token.")]
    [SwaggerResponse(StatusCodes.Status200OK, "Operación exitosa", typeof(AuthenticationResponse))]
    [SwaggerResponse(StatusCodes.Status400BadRequest, "Solicitud incorrecta, El token se encuentra vigente",
        typeof(string))]
