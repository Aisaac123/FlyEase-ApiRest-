...
public class ClientesController : CrudController<Cliente, string, FlyEaseDataBaseContextAuthentication>
{
    ...
    public ClientesController(FlyEaseDataBaseContextAuthentication context, IHubContext<WebSocketHub> hubContext) :
    ...
    protected override async Task<string> InsertProcedure(Cliente entity)
    {
        try
        {
            ...
            {
                new("v_numerodocumento", entity.Numerodocumento),
                new("v_tipodocumento", entity.Tipodocumento),
                new("v_nombres", entity.Nombres),
                new("v_apellidos", entity.Apellidos),
                new("v_celular", entity.Celular),
                new NpgsqlParameter("v_correo", entity.Correo)
            };

            await _context.Database.ExecuteSqlRawAsync(
                "CALL p_insertar_cliente(@v_numerodocumento, @v_tipodocumento, @v_nombres, @v_apellidos, @v_celular, @v_correo)",
                parameters);
            return "Ok";
        }
        catch (Exception ex)
    ...
    protected override async Task<string> UpdateProcedure(Cliente entity, string OldId)
    {
        try
        {
            var parameters = new NpgsqlParameter[]
            {
                new("numero_documento_cliente", OldId),
                new("nuevo_documento", entity.Numerodocumento),
                new("nuevo_tipo_documento", entity.Tipodocumento),
                new("nuevo_nombres", entity.Nombres),
                new("nuevo_apellidos", entity.Apellidos),
                new NpgsqlParameter("nuevo_celular", entity.Celular),
                new NpgsqlParameter("nuevo_correo", entity.Correo)
            };

            await _context.Database.ExecuteSqlRawAsync(
                "CALL p_actualizar_cliente(@numero_documento_cliente, @nuevo_documento, @nuevo_tipo_documento, @nuevo_nombres, @nuevo_apellidos, @nuevo_celular, @nuevo_correo)",
                parameters);
            return "Ok";
        }
        catch (Exception ex)
    ...
    protected override async Task<string> DeleteProcedure(string Old_Id)
    {
        try
        {
            var parameters = new NpgsqlParameter[]
            {
                new("numero_documento_cliente", Old_Id)
            };

            await _context.Database.ExecuteSqlRawAsync("CALL p_eliminar_cliente(@numero_documento_cliente) ",
                parameters);
            return "Ok";
        }
        catch (Exception ex)
    ...
    [SwaggerResponse(StatusCodes.Status401Unauthorized, "Sin autorizacion, por favor solicite el token",
        typeof(string))]
        ...
        try
        {
            ...
            if (entity != null)
                return StatusCode(StatusCodes.Status200OK,
                    new { mensaje = "ok", Succes = true, response = entity.Boletos });
            return StatusCode(StatusCodes.Status400BadRequest,
                new { mensaje = "Cliente no encontrado", Succes = false, response = new List<Asiento>() });
        }
        catch (Exception ex)
        {
