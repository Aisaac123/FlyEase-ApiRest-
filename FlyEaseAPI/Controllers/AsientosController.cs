...
public class AsientosController : CrudController<Asiento, int, FlyEaseDataBaseContextAuthentication>
{
    ...
    public AsientosController(FlyEaseDataBaseContextAuthentication context, IHubContext<WebSocketHub> hubContext) :
    ...
    protected override async Task<string> InsertProcedure(Asiento entity)
    {
        try
        {
            ...
            {
                new("v_posicion", entity.Posicion),
                new("v_disponibilidad", entity.Disponibilidad),
                new("v_idcategoria", entity.Categoria.Idcategoria),
                new("v_idavion", entity.Avion.Idavion)
            };

            await _context.Database.ExecuteSqlRawAsync(
                "CALL p_insertar_asiento(@v_posicion, @v_disponibilidad, @v_idcategoria, @v_idavion)", parameters);
            return "Ok";
        }
        catch (Exception ex)
        {
    ...
    protected override async Task<string> UpdateProcedure(Asiento nuevoAsiento, int id_asiento)
    {
        try
        {
            ...
            {
                new("id_asiento", id_asiento),
                new("nueva_posicion", nuevoAsiento.Posicion),
                new("nueva_disponibilidad", nuevoAsiento.Disponibilidad),
                new("nuevo_id_categoria", nuevoAsiento.Categoria.Idcategoria),
                new("nuevo_id_avion", nuevoAsiento.Avion.Idavion)
            };

            await _context.Database.ExecuteSqlRawAsync(
                "CALL p_actualizar_asiento(@id_asiento, @nueva_posicion, @nueva_disponibilidad, @nuevo_id_categoria, @nuevo_id_avion)",
                parameters);
            return "Ok";
        }
        catch (Exception ex)
