...
namespace FlyEase_ApiRest_.Controllers
{
    namespace TuNamespaceDeControladores
    {
        ...
        public class AeropuertosController : CrudController<Aeropuerto, int, FlyEaseDataBaseContextAuthentication>
        {
            ...
            public AeropuertosController(FlyEaseDataBaseContextAuthentication context,
                IHubContext<WebSocketHub> hubContext) : base(context, hubContext)
            {
                _context = context;
            }
            ...
            protected override async Task<string> InsertProcedure(Aeropuerto entity)
            {
                try
                {
                    ...
                    {
                        new("nombre_pais", entity.Ciudad.Region.Pais.Nombre),
                        new("nombre_region", entity.Ciudad.Region.Nombre),
                        new("nombre_ciudad", entity.Ciudad.Nombre),
                        new("v_latitud", entity.Coordenadas.Latitud),
                        new("v_longitud", entity.Coordenadas.Longitud),
                        new("nombre_aereopuerto", entity.Nombre),
                        v_imagen
                    };

                    await _context.Database.ExecuteSqlRawAsync(
                        "CALL p_insertar_aereopuerto(@nombre_pais, @nombre_region, @nombre_ciudad, @v_latitud, @v_longitud, @nombre_aereopuerto, @_imagen)",
                        parameters);
                    return "Ok";
                }
                catch (Exception ex)
            ...
            protected override async Task<string> DeleteProcedure(int id_Aeropuerto)
            {
                try
                {
                    var parameters = new NpgsqlParameter[]
                    {
                        new("id_aereopuerto", id_Aeropuerto)
                    };

                    await _context.Database.ExecuteSqlRawAsync("CALL p_eliminar_aereopuerto(@id_aereopuerto)",
                        parameters);
                    return "Ok";
                }
                catch (Exception ex)
            ...
            protected override async Task<string> UpdateProcedure(Aeropuerto nuevoAeropuerto, int id_Aeropuerto)
            {
                try
                {
                    ...
                    {
                        new("id_aereopuerto", id_Aeropuerto),
                        new("nuevo_nombre", nuevoAeropuerto.Nombre),
                        new("nueva_ciudad", nuevoAeropuerto.Ciudad.Nombre),
                        new("nueva_region", nuevoAeropuerto.Ciudad.Region.Nombre),
                        new("nuevo_pais", nuevoAeropuerto.Ciudad.Region.Pais.Nombre),
                        new("nueva_latitud", nuevoAeropuerto.Coordenadas.Latitud),
                        new NpgsqlParameter("nueva_longitud", nuevoAeropuerto.Coordenadas.Longitud),
                        v_imagen
                    };

                    await _context.Database.ExecuteSqlRawAsync(
                        "CALL p_actualizar_aereopuerto(@id_aereopuerto, @nuevo_nombre, @nueva_ciudad, @nueva_region, @nuevo_pais, @nueva_latitud, @nueva_longitud, @_imagen)",
                        parameters);
                    return "Ok";
                }
                catch (Exception ex)
