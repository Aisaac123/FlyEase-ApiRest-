...
public class RegionesController : CrudController<Region, int, FlyEaseDataBaseContextAuthentication>
{
    ...
    public RegionesController(FlyEaseDataBaseContextAuthentication context, IHubContext<WebSocketHub> hubContext) :
    ...
    protected override async Task<string> InsertProcedure(Region entity)
    {
        try
        {
            ...
            {
                new("nombre_region", entity.Nombre),
                new("nombre_pais", entity.Pais.Nombre)
            };

            await _context.Database.ExecuteSqlRawAsync("CALL p_insertar_region(@nombre_region, @nombre_pais)",
                parameters);
            return "Ok";
        }
        catch (Exception ex)
    ...
    protected override async Task<string> UpdateProcedure(Region nuevaRegion, int id_region)
    {
        try
        {
            ...
            {
                new("id_region", id_region),
                new("nuevo_nombre", nuevaRegion.Nombre),
                new("nuevo_id_pais", nuevaRegion.Pais.Idpais)
            };

            await _context.Database.ExecuteSqlRawAsync(
                "CALL p_actualizar_region(@id_region, @nuevo_nombre, @nuevo_id_pais)", parameters);
            return "Ok";
        }
        catch (Exception ex)
        {
