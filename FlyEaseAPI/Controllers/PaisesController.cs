...
public class PaisesController : CrudController<Pais, int, FlyEaseDataBaseContextAuthentication>
{
    ...
    public PaisesController(FlyEaseDataBaseContextAuthentication context, IHubContext<WebSocketHub> hubContext) : base(
        context, hubContext)
    {
        _context = context;
    }
    ...
    protected override async Task<string> UpdateProcedure(Pais nuevoPais, int id_pais)
    {
        try
        {
            ...
            {
                new("id_pais", id_pais),
                new("nuevo_nombre", nuevoPais.Nombre)
            };
