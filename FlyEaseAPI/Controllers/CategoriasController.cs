...
public class CategoriasController : CrudController<Categoria, int, FlyEaseDataBaseContextAuthentication>
{
    ...
    public CategoriasController(FlyEaseDataBaseContextAuthentication context, IHubContext<WebSocketHub> hubContext) :
    ...
    protected override async Task<string> InsertProcedure(Categoria entity)
    {
        try
        {
            ...
            {
                new("v_nombre", entity.Nombre),
                new("v_descripcion", entity.Descripcion),
                new("v_estadocategoria", entity.Estadocategoria),
                new("v_tarifa", entity.Tarifa),
                new("v_comercial", entity.Comercial)
            };

            await _context.Database.ExecuteSqlRawAsync(
                "CALL p_insertar_categoria(@v_nombre, @v_descripcion, @v_estadocategoria, @v_tarifa, @v_comercial)",
                parameters);
            return "Ok";
        }
        catch (Exception ex)
    ...
    protected override async Task<string> UpdateProcedure(Categoria nuevaCategoria, int id_categoria)
    {
        try
        {
            ...
            {
                new("id_categoria", id_categoria),
                new("nuevo_nombre", nuevaCategoria.Nombre),
                new("nueva_descripcion", nuevaCategoria.Descripcion),
                new("nuevo_estado_categoria", nuevaCategoria.Estadocategoria),
                new("nueva_tarifa", nuevaCategoria.Tarifa),
                new NpgsqlParameter("nuevo_comercial", nuevaCategoria.Comercial)
            };

            await _context.Database.ExecuteSqlRawAsync(
                "CALL p_actualizar_categoria(@id_categoria, @nuevo_nombre, @nueva_descripcion, @nuevo_estado_categoria, @nueva_tarifa, @nuevo_comercial)",
                parameters);
            return "Ok";
        }
        catch (Exception ex)
